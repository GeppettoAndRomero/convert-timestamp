...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/convert-timestamp/sw.js, scope=/convert-timestamp/）。
    // scope がツールの全 locale ページ（/convert-timestamp/, /convert-timestamp/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/convert-timestamp/sw.js', {
